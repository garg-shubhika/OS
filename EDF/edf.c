...
if (verbose) 
{
  for(int i = 1; i < nbOfTasks; i++) 
  {
    printf("Task: %s  \tarrivalDate:%d \tstate:%s  \tcomputations:%d/%d \tdeadline_time:%d\n",
	   ...
	   ((tasks[i].number_of_executions + 1) * tasks[i].deadline) - cur_time);
  }
}
}
...
int EDF(task tasks[], int nbOfTasks, int cur_time) 
{
  ...
  for (int i = 1; i < nbOfTasks; i++) 
  {
    if (tasks[i].state == ADMISSION) 
	{
	 ...
	 next = (tasks[i].number_of_executions + 1) * tasks[i].deadline;
     if (cur_time == next) 
	 {
	  ...
	  {
		min_deadline = ((tasks[i].number_of_executions + 1) * tasks[i].deadline) - cur_time;
		val = 1;
        scheduled_task = i;
      }
      if (tasks[i].state == RUNNABLE) 
	  {
        deadline_time = ((tasks[i].number_of_executions + 1) * tasks[i].deadline) - cur_time;
